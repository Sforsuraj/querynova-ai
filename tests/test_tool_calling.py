...
def test_registry_only_exposes_expected_tools():
    state = {}
    tools = DataAnalystAgent()._tool_registry(state)
    assert set(tools.keys()) == {'execute_query', 'get_schema', 'generate_chart', 'generate_flowchart'}
...
def test_tool_call_loop_executes_schema(monkeypatch):
    ...
    call = SimpleNamespace(
        ...
        model_dump=lambda: {'id': 'call_1', 'type': 'function', 'function': {'name': 'get_schema', 'arguments': '{}'}}
    )
    ...
    completions = [
        SimpleNamespace(choices=[SimpleNamespace(message=tool_message)], model='test-model'),
        SimpleNamespace(choices=[SimpleNamespace(message=final_message)], model='test-model')
    ]
